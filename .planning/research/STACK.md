# Stack Research: Gold Advisor Vietnam

**Domain:** AI-powered financial advisory web app (gold price tracking, signal generation, dashboard, messenger alerts)
...
# Database
uv add sqlalchemy[asyncio] aiosqlite alembic duckdb

# Scheduling
uv add "APScheduler>=3.11.2,<4.0"

# Data fetching & scraping
uv add httpx beautifulsoup4 lxml "playwright>=1.49"
...
**If Vietnamese gold shop sites are mostly static HTML:**
...
**If Vietnamese gold shop sites use heavy JavaScript (React/Vue SPA):**
...
**If you need to handle anti-bot protection:**
...
**If you later want multi-LLM support:**
...
**If data volume grows beyond SQLite's comfort zone (>10GB, concurrent writes):**
...
│                    FastAPI Application               │
│  ┌──────────┐  ┌───────────┐  ┌──────────────────┐  │
│  │ Dashboard │  │ REST API  │  │ APScheduler (3.x)│  │
│  │ (Jinja2)  │  │ Endpoints │  │ - Scrape jobs    │  │
│  └──────────┘  └───────────┘  │ - Signal analysis │  │
│       │              │         │ - Alert dispatch  │  │
│       └──────────────┴─────────┴────────┬─────────┘  │
│                                          │            │
...
│  │                   Service Layer                  │ │
│  │  ┌────────────┐ ┌──────────┐ ┌───────────────┐  │ │
│  │  │ Scrapers   │ │ Analysis │ │ LLM Advisor   │  │ │
│  │  │ (httpx+BS4 │ │ Engine   │ │ (OpenAI)      │  │ │
│  │  │ +Playwright│ │ (pandas) │ │               │  │ │
│  │  └────────────┘ └──────────┘ └───────────────┘  │ │
│  └───────────────────────┬─────────────────────────┘ │
│                          │                            │
...
│  │                   Data Layer                     │ │
│  │  ┌──────────┐  ┌───────────┐  ┌──────────────┐  │ │
│  │  │ SQLite   │  │ DuckDB    │  │ Alerts Queue │  │ │
│  │  │ (writes) │  │ (analytics│  │ (Telegram +  │  │ │
│  │  │          │  │  queries) │  │  Zalo OA)    │  │ │
│  │  └──────────┘  └───────────┘  └──────────────┘  │ │
│  └──────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────┘
...
1. **Ingestion** (APScheduler cron, every 5-15 min): Scrapers fetch prices from Vietnamese shops (httpx/Playwright) and international APIs (httpx). Raw data validated via Pydantic, stored in SQLite.
2. **Analysis** (APScheduler, every 15-30 min): Analysis engine reads from SQLite, uses pandas to compute SJC-international gap, buy/sell spreads, moving averages. Results stored back in SQLite.
3. **Signal Generation** (triggered by analysis): If gap/spread crosses thresholds, generate buy/sell/hold signal with confidence level. Send to LLM advisor for natural language explanation (in Vietnamese).
4. **Alert Dispatch** (triggered by signal): Push formatted alert to Telegram bot and/or Zalo OA API via httpx.
5. **Dashboard** (user-initiated): FastAPI serves Jinja2 templates. API endpoints serve JSON for charts. DuckDB handles analytical queries (historical gaps, trends).
...
---
*Stack research for: Gold Advisor Vietnam (AI financial advisory app)*
*Researched: 2026-03-25*
